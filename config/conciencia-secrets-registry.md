...
| OpenRouter API key (ConcienciaTM's own) | `.env.conciencia` (`OPENROUTER_API_KEY`) on the deploy host | **must be its own key, never EBM's or City's** — pending emission (F1.10) |
...
- [ ] ConcienciaTM's own OpenRouter key: not yet emitted (blocked on F1.10).
